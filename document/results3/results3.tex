...
RSMF (root mean squared fluctuations) of the residues along each chain in the protofibrils show that the protofibrillar aggregate is dynamic in the simulation [Still vague be more specific, why is this significant?] (Figure: RMSF)
...
\textbf{Key result}. Does inositol, glucose and glycerol, preferentially partition to different binding sites on the protofibrillar surface? Based on the global binding mode as shown in the spatial distribution suggest that this is the case. The global binding modes suggests that the active ligand, scyllo-inositol is able to bind the KLVFFAE face better. This can be related to fibril inhibition because this region is known for the initiation of aggregation. Furthermore, all of my models are converging on similar answers (that is binding to protofibrils, and binding to KLVFFAE segment), which is reassuring!
Different binding modes between scyllo- and chiro-inositol.  Perhaps this is a reason why scyllo works better is that it adopts more inhibition-productive binding modes. It makes more specific interactions as indicated by our previous work on KLVFFAE. Chiro binding in the tunnel is an example just gets trapped in ``non-productive'' binding modes.

TODO: Calculate the nonpolar and polar binding modes for each of these (that is the pie charts in the KLVFFAE study). If there is a difference, I can then extrapolate and correlate these differences to the SAR of inositol.
...
Scyllo-inositol preferentially bind the face containing the residues LVFFA, which are residues at the central hydrophobic core of Abeta. [Elaborate]
By contrast, chiro-inositol and glucose molecules predominantly bind to the beta2 face. [Elaborate] As depicted in Figure XXX, there is no binding densities of glycerol at the surfaces of the protofibril at an occupancy of YYY, suggesting that glycerol does not significantly bind the solvent-exposed surface of the protofibril.
...
Summary and discussion of how the binding modes observed in our simulation studies here relates to the Abeta amyloid inhibition. Our results suggest that the most likely mechanism of action of scyllo-inositol is to bind the faces of the protofibril formed from the CHC residues.
...
TODO: In addition to stereochemistry-dependent activity, inositol was found to be inactive for inhibiting the amyloid formation of Abeta40 - put forth a few speculations based on the data of my current study to explain 
