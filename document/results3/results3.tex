...
In two previous studies (Chapters 3 and 4), we have successively examined the binding mechanism of scyllo- and chiro-inositol with peptide and aggregate states of model amyloid-forming peptides\cite{Li:2012bx} and A$\beta$(16-22).\cite{Li:2013fo}
...
have observed similar binding modes for amyloid dye molecules (ThT and PiB),\cite{Wu:2011fd} and a polyphenol morin.\cite{Lemkul:2010p6251}
% TODO AND what???
...
In corroboration with our results, a recent simulation study by Convertino et al.\cite{convertino} found that anthraquinone was likely to act by binding to LVFFA region. 

On the basis of our results above, we hypothesize that the CHC, particularly the residues Phe20 and Glu21, is a critical binding site of \textit{scyllo}-inositol for preventing the formation of A$\beta$42 fibrils. 
...
Depending on the morphology of the amyloidogenic aggregate, the binding modes can be different. In our previous study, we examined binding with KLVFFAE. Unlike A$\beta$42, the faces and grooves of the protofibril of A$\beta$(16-22) are chemically identical: each face consists of the same polar, charged and hydrophobic grooves. Hence, differences in binding between \textit{scyllo}- and \textit{chiro} was not revealed by examining the protofibril of A$\beta$(16-22). \textbf{Although binding modes of a small molecule may remain consistent across relatively different models, the binding site and specificity of a particular small molecule can be strongly modulated by the fibril surface properties.} As a consequence, comparatively examining the binding modes of several small molecules with differing activities is XXX for determining the binding modes and sites of a potential drug.  Also as a consequence although utilizing model peptides may inform of residue-specific binding modes for a small molecule, it may miss out on picking out the correct binding sites as the activity of small molecule inhibitors are modulated by the sequence and morphology of its binding partner.
