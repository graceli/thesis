...
Using quantum mechanical theory, the time-evolution of a molecular system can be predicted by solving the time-dependent Schrodinger's equation, which provides a complete description of the motion of atomic-nuclei and electronic from first-principles.  However,  such an equation can only be solved analytically for a single hydrogen atom, and numerically using computational algorithms for very small systems. Many quantum-mechanics-based methods to probe biomolecular systems have been developed, but they are computationally too prohibitive to structures as big as proteins (which typically possess on the order of 10$^{3}$ number of atoms) on a biologically-meaningful timescale. REF

In the past decades, molecular mechanics, an approach which treats a molecular system using classical mechanics was developed, which made modelling and simulating the molecular dynamics of much larger biomolecular systems possible. In molecular mechanics, a molecule is essentially represented a set of masses (atoms) connected by springs (bonds). An empirical potential energy function (PEF) that depends only on the positions of the atoms in the system, is employed to describe the internal energy and interactions of molecules.

The theoretical basis for molecular mechanics is rooted in quantum mechanics via the Born-Oppenheimer approximation. 
Under this approximation, electronic and nuclear motion of atoms are considered to be uncoupled, and therefore can be treated separately. For this reason, a system's total energy, or its Hamiltonian, $H$, only depends on the electronic motion. This is because the nuclei, which are much heavier than electrons, are typically fixed on the time scales of electronic vibration.

The potential energy function employed in molecular mechanics corresponds to the Born-Oppenheimer energy surface of the molecular system in its electronic ground-state. The movements of all atomic nuclei are treated as a point charge in space, where electronic motion is implicitly accounted for in the specific form of the PEF and its associated parameter set.  Because electronically excited states are not treated, no chemical reactions occur in the system.
