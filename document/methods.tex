...
\textbf{
Under the Born-Oppenheimer approximation, electronic and nuclear motions are considered to be uncoupled, and therefore are treated separately, where the system's total energy (the Hamiltonian $H$) only depends on the electronic motions, where as the nuclear positions are assumed to be fixed. The basis for this approximation is that the nuclei, which are much heavier than electrons, are typically fixed on the timescales of electronic vibration.

% \cite{M. Born and R. Oppenheimer, Quantum Theory of Molecules, Ann. Physics, vol. 84, pp. 457‚484, 1927.} -- can't find this article.

% For this reason, the movements of all atomic nuclei can be treated as classical particles (that is, an atom is a point charge in space), where the movement of electrons are not accounted for. Instead, the effects of electronic motion is implicitly accounted for in the force field (described in more detail below). 

% Because electronically excited states are not treated, no chemical reactions occur in the system.

In molecular mechanics, an empirical potential energy function is employed to describe the molecular energies of molecules.  

This potential energy function is based on the Born-Oppenheimer energy surface of the molecule in its electronic ground-state.  

Electrons can then be considered as implicit variables of this potential function. 

describing the connectivity of atoms masses being connected by springs.

MD is a molecular mechanics simulation technique.

MM: In biomolecular systems, covalent bond stretching and angle-bending motions are treated as harmonic.
}
...
Here, describe the functional terms which is used to describe bonded interactions
