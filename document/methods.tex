...
Bond torsions are typically accounted for using a periodic function,
\begin{equation}
  \label{eq:torsion}
   V_{dihedral} = \sum_{dihedrals} k_{\chi}(1 + cos(n\chi - \delta)),
\end{equation}
where $V_{dihedral}$ is the first term of a Fourier expansion, $n$ denotes the periodicity of the function and $\delta$ is the phase. Moreover, dihedrals of this form (improper dihedrals) may be added to the PEF as a way of ensuring the planarity of molecules such as those with rings.
...
As previously discussed in Chapter 1, hydrogen bonding is an important type of electrostatic interaction and hence their proper representation must be captured by a force field. An explicit term solely for describing hydrogen bonding have been employed by some force fields. For example, the original AMBER force field introduced in 1984 added a function of the form $\sum_{hbonds} C_{ij}/R_{ij}^{12} - D_{ij}/R_{ij}^{10}$ to account for hydrogen bonds.\cite{Weiner:1984uw}  However, it was later demonstrated that hydrogen bonding interactions are accurately accounted for via the combination of van der Waals and electrostatic potential terms. Hence, most modern force fields for protein simulations, including the latest AMBER force fields, do not utilize an explicit hydrogen bonding term.
...
In this section, I briefly describe two of the most popular force fields that are used for protein simulations, AMBER and OPLS force fields. OPLS-AA and AMBER99 were used to generate the results of this thesis presented in Chapters 3 to 5, and Chapter 6, respectively.  Detailed and extensive reviews of empirical force fields for protein simulations have been provided elsewhere.\cite{Ponder:2003uw,MackerellJr:2004p5558}

One of the first generation of force fields developed for the simulations of proteins was the AMBER force field, which was developed along with the Amber molecular mechanics software package in the Kollman group.  The central ideas for parameterization in Amber was using quantum mechanical calculations to fit partial charges to the quantum electrostatic potential (ESP).   The parameters for the van der Waals potential terms were fit to amide crystal data and liquid-state simulations.  \textbf{Force constants and idealized bond lengths and angles were taken from crystal structures and adapted to match normal mode frequencies for a number of peptide fragments. Finally, torsion force constants were adjusted to match torsional barriers extracted from experiment or from quantum chemistry calculations.}  Later, an all-atom version of Amber was developed due to problems with the polar-hydrogen only force fields. 
...
An all-atom OPLS force field (OPLS-AA) was developed for organic molecules and peptide simulations in 1996.\cite{Jorgensen:1996vx} Although the charge and van der Waals parameters were developed from simulations of pure liquids, which is unique to the parameterization of OPLS, the atom types and parameters for bonded interactions (bonds, angles, and dihedrals) were largely adopted from the AMBER all-atom force field.\cite{Jorgensen:1996vx}

The OPLS-AA potential energy function is
