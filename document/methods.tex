...
A typical empirical force field has many parameters to be determined. 

To fit these parameters, one approach is to perform quantum mechanical calculations ...  

Once the initial parameters are determined, force fields are iteratively improved by predicting experimentally observable quantities for small compounds, and adjusting the fit based on comparisons of these computationally predicted quantities with experimental measurements.

Currently many force fields for the purposes of simulating biomolecules exist. Examples are AMBER (and related versions and modifications), CHARMM, GROMOS, OPLS.  Although each force field have their variation of the PEF form, force fields in general adopt the following form:
...
The first 4 terms in \ref{eq:ff_general} are the functional terms used to describe bonded interactions within a molecule.
Covalent bond stretching and angle-bending motions are both treated as harmonic.

\subsection{Non-bonded interactions}

van der Waals forces is represented by the Lennard-Jones potential
...
The first term of \ref{eq:lj_function} is an attractive term and the second term represents a repulsive force at  atomic distances where atomic shells overlap.

Electrostatics. The reproduction of electrostatics is paramount for the molecular interaction of polar and charged molecules. To do so the Coulombic potential

\begin{equation}
  \label{eq:electrostatic}
  V_{coulomb} = \frac{q_1q_2}{er}
\end{equation}

is applied between atoms at their nuclear centres.
