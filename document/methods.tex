...
In principle, using quantum mechanics, the time-evolution of a molecular system can be predicted by solving the time-dependent Schr\"odinger's equation, which provides a complete description of the motion of atomic nuclei and electrons from first-principles.\cite{Schrodinger:1926ka} Notably, the solution for this equation can only be obtained analytically for a hydrogen atom, and must be solved numerically for more complex systems. Although many quantum-mechanics-based computational methods have been developed in recent years,\cite{Gogonea:2001us} they are still computationally prohibitive for probing the dynamics of entire protein systems on a biologically-meaningful timescale because these systems, with solvent molecules included, typically possess on the order of 10$^{4}$ to 10$^{5}$ number of atoms.

Molecular mechanics is an approach in which a molecular system is treated using the principles of classical mechanics. In molecular mechanics, a molecule is modelled by a set of masses (atoms) connected by springs (bonds).  An empirical potential energy function (PEF) that only depends on the positions of the atoms in the system is employed to describe the internal energy and interactions of molecules.\cite{Cornell:1995td,Brooks:1983uc,Christen:2005fp,Jorgensen:1985wx} The theoretical basis for molecular mechanics is rooted in quantum mechanics via the Born-Oppenheimer approximation.\cite{Born:1927tb} Under this approximation, the electronic and nuclear motion of atoms are considered to be uncoupled, and therefore can be treated separately.
% Note that the previous sentence is a little vague -- what does it mean to be ``treated separately''
Because nuclei are much heavier than electrons, they are typical fixed on the time scales of electronic vibration. For this reason, the total energy (or the Hamiltonian, $H$) of a system can be considered to depend only on the electronic motion. 
...
The potential energy function employed in molecular mechanics corresponds to the Born-Oppenheimer energy surface of the molecular system in its electronic ground-state.  % can't find a ref for this and all the places where I see this reference, I don't see a corresponding reference. 
In molecular mechanics, atomic nuclei are treated as point charges, where the motion of their electrons is implicitly accounted for in the PEF.  Because electronically excited states are not treated, no chemical reactions (i.e., bond formation) can occur.
...
The functional form of the potential energy function together with its associated parameters define a \emph{force field}.  A typical empirical force field has many parameters to be determined.  These parameters are determined by fitting to quantum mechanical calculations, and to physical measurements (e.g., using data from X-ray crystallography, neutron diffraction, vibrational raman spectroscopy) of small organic compounds or building blocks of peptides and nucleic acids. Force field parameters are adjusted to improve the agreement of predicted quantities with \emph{ab initio} quantum mechanical calculations, and experimental data. In the past four decades, a variety of force fields have been developed and applied towards the modelling of biomolecular systems involving proteins, nucleic acids and carbohydrates. The widely adopted force fields include AMBER,\cite{Cornell:1995td} CHARMM,\cite{Brooks:1983uc} GROMOS,\cite{Christen:2005fp} and OPLS.\cite{Jorgensen:1985wx,Jorgensen:1996vx,Kony:2002ji} Reviews and comparisons of these force fields are provided elsewhere.\cite{Beauchamp:2012eq,MackerellJr:2004p5558,Ponder:2003uw}  Although each of these force fields have different parameters, they share a similar general form of the PEF:
\begin{equation}
\label{eq:ff_general}
...
The first four functional terms of Equation~\ref{eq:ff_general}, $V(bonds)$, $V(angles)$,  $V(dihedral)$, and $V(improper)$, define the through-bond interactions, covalent bond stretching, angle-bending, and bond torsion angles, and planar dihedral angles, respectively. Covalent bond stretching can be described using Hooke's Law for harmonic motion,
\begin{equation}
\label{eq:bonds}
...
In Equations~\ref{eq:bonds} and \ref{eq:angles}, the constants $k_{b}$ and $k_{\theta}$ are the equilibrium force constants for bonds and angles, respectively, and the constants $b_{0}$ and $\theta_{0}$ are the reference bond length and angle, respectively. The total energetic contribution from bond stretching and angle bending are computed by summing over all bonds and angles in the system. In addition, the torsion (or dihedral) angle of bonds are also important for describing molecular conformations.  These energetic terms are commonly accounted for using a periodic function of the form,
\begin{equation}
\label{eq:torsion}
	V(dihedrals) = \sum_{dihedrals} \sum_{n} k_{\chi}(1 + cos(n\chi - \delta)),
\end{equation}
where $k_{\chi}$ and $n$ is the height and the periodicity of the rotational barrier, respectively.  $\delta$ is the reference angle (or phase).  The value of $n$ is usually between 1 and 4, and depends on the specific force field parameterization procedure. Functions of this form (the so-called improper dihedrals) are also included the PEF to ensure the planarity of planar chemical groups.


\subsection{Non-bonded Interactions}
Non-bonded interaction potentials predominantly account for van der Waals and electrostatic interactions.  Accurate reproduction of these interactions are particularly important for the transferability of a force field, which is the assumption that a force field developed based on a small set of molecules can predict properties of a broader set of molecules composed of similar chemical groups. The non-bonded forces are only applied to atom pairs separated by at least three bonds, and are pair-wise additive, that is, the total contribution to the potential energy from the non-bonded forces is the sum total over all interacting atomic pairs in a system. Van der Waals interactions, are represented by the Lennard-Jones potential,
...
The first term of Equation~\ref{eq:lj_function} represents a repulsive force at distances where atomic shells overlap. The second term represents the attractive force between atoms which is most favorable at a distance of $r_{min}$, the preferred interaction distance for a pair of atoms. $V_{LJ}$ is a dependent on the distance between atoms, $r$, and decays rapidly to 0 at short ranges.

The reproduction of electrostatics is paramount for describing interactions of polar and charged groups in biomolecules. To do so, the Coulombic potential,
\begin{equation}
  \label{eq:electrostatic}
  V_{coulombic}(r) = \frac{q_iq_j}{\epsilon r}, 
\end{equation}
is calculated for all atom pairs which carry partial charges or charges at their nuclear centers.  In Equation~\ref{eq:electrostatic}, $q_{i}$ and $q_{j}$ are the point charges assigned to atoms $i$ and $j$, respectively, and $\epsilon$ is the dielectric constant.  In contrast to van der Waals interactions,  electrostatic interactions can be significant for charged pairs that are separated by longer distances. Accordingly, in comparisons to the Lennard-Jones potential, the Coulombic potential decays to zero at a much slower rate. 
%\begin{figure}
% \centering
...
Polarizability is the measure of the change in a molecule's electron distribution in response to an applied electric field, and is a property of matter which can be induced by electrostatic interactions with solvents or ionic reagents.  As mentioned above, most current empirical force fields use a fixed-charge model where each atom in the system is assigned a single value that remains unchanged.  Polarizable force fields which account for variations in charge distribution with dielectric environment, have been introduced.\cite{Halgren:2001waa,Warshel:2007p5320} However, because of the associated high computational cost of using these force fields, they are not routinely used for biomolecular simulations.
...
At a very high level, MD simulation is an algorithm that simulates motion of a system under the influence of a specific force field by following molecular configurations in time according to Newton's equation of motion. 

% This is the part with the time propagation using the Equations of motion
In a system composed of $N$ interacting particles, the force on each particle is determined by taking the spatial derivative of the potential energy of the system, $V(r_{1}, r_{2}, ... , r_{N})$.  In MD simulations, this potential energy is approximated by a molecular mechanics force field as described above.  The force on each particle $i$ is given by
...
The velocity of atom $i$, $v_{i}(t)$, is then given by,
\begin{equation}
v_i(t+\delta t) = v_i(t) + \int_{t}^{t+\delta t} a_{i}(t) dt = v_i(t) + a_{i}\cdot \delta t.
\end{equation}
The position of atom $i$ at time $ t + \delta t$ is then,
\begin{equation}
x_i(t + \delta t) = x_i(t) + v_i(t)\delta t + \frac{a_i(t)\delta t^2}{2}.
\end{equation}
The positions of each of the $N$ particles for $N \geq 3$ are approximated numerically in MD simulations using a time-integration algorithm. For example, the leap-frog and velocity-verlet algorithms are commonly used in MD simulations.\cite{Hockney:1974tu,Swope:1982tq} Once the position of each particle is computed, subsequent interatomic forces are updated based on these new coordinates, and the entire process is repeated to obtain the coordinates of the system at future time steps. To ensure numerical stability, a small integration time step in the range of 1 - 4 femtoseconds (fs) is chosen. In MD simulations of biomolecular systems, a 2 fs timestep is commonly chosen because it is twice the time period for the fastest vibrational motion (existing in bonds involving hydrogen). % [Ref: Chris Madill's and Tom's thesis]
...
\end{equation}
where $k_{B}$ is the Boltzmann constant. In MD simulations, the temperature of the system is the total kinetic energy of the system normalized by the number of degrees of freedom in the system, $N_{f}$, that is,
\begin{equation}
  T(t) = \sum_{i=1}^{N} \frac{m_iv_i^2(t)}{k_BN_f},
\end{equation}
where $N$ is the number of atoms, and $v_{i}(t)$ is the velocity of atom $i$. To simulate a system close to laboratory conditions,  a thermostat and a barostat (a virtual pressure piston) are employed to control both the temperature and pressure of a simulation system, respectively.  Several different temperature-\cite{Berendsen:1984p9257,Nose:1984em,Hoover:1985wf} and pressure-coupling algorithms\cite{Nose:1983vt,Parrinello:1981vy} exist and are widely implemented across modern molecular dynamics simulation software packages.
...
Periodic boundary conditions (PBC) is a technique by which the central simulation cell (or ``box'') is replicated along each of its dimensions to form a system with infinite number of images, and is used to alleviate artificial phase boundaries imposed by a finite simulation cell.  Furthermore, PBC allows the simulation system to be treated as a bulk system, akin to \textit{in vitro} experimental conditions.  To avoid counting duplicate interactions from multiple adjacent cells, a minimum image convention is applied -- an atom $i$ interacts with only another atom $j$, provided that $j$ is in $i$'s closest periodic image.

\subsection{Theoretical Basis of MD Simulations}
MD simulations yield information about the microscopic states of a macroscopic system characterized by thermodynamic variables such as temperature, pressure, and the number of particles.   These variables are what is typically observed and measured experimentally.  Statistical mechanics is a body of theory that provides a rigorous connection between the microscopic arrangements of a system (determined by MD simulations) to its thermodynamic properties.

MD simulations can be carried out in the canonical (NVT) ensemble, where the number of particles (N), volume (V) and temperature (T) are constants, or alternatively in the isothermo-isobaric (NPT) ensemble, where the system has fixed number of particles and temperature, but its average pressure is held constant by allowing the volume of the simulation box to fluctuate. Because the NPT ensemble is the closest to experimental conditions, production simulations are often carried out in this ensemble.
...
% Z = e ^{\frac{-A}{k_{B}T}}
\end{equation}
$Z = \sum_{i} e ^{\frac{-E_{i}}{kT}}$ is the normalization factor (or the \emph{partition function}) such that $\sum_{i} p_{i} = 1$.
Any thermodynamic variable can then be derived from the partition function. For example, free energy, an important thermodynamic quantity, measures the useful work obtainable from a system under a certain set of conditions.  It is called the Gibbs free energy (G) and Helmholtz free energy (A) for a system in the NPT and NVT ensemble, respectively. In biochemistry, a central quantity of interest is the change in free energy between two states of interest, which measures whether a reversible biochemical process will occur spontaneously.  $\Delta G$ for any two states, 0 and 1, is related to their respective probabilities $p_{0}$ and $p_{1}$ via
\begin{equation}
\Delta G = - k_{B}Tln \frac{p_{0}}{p_{1}}.
\end{equation}

Performing an MD simulation yields a time-trajectory of the positions of atoms in the system from which we can obtain time-averaged thermodynamic properties of the system.  However, in an experiment, the macromolecular observables measured are a result of an average over all possible microstates of a system, or an \emph{ensemble average}.  This apparent conundrum is reconciled by one of the fundamental axioms of statistical mechanics, the ergodic hypothesis, which states that if a system is given long enough time to evolve, the time average of its macromolecular properties is equivalent to their ensemble averages.  In practice, this means that if a MD simulation sufficiently samples the relevant regions of the phase space (i.e., the collection of all possible configurations of a given system), then statistically-meaningful properties can be extrapolated from such a simulation.

\subsection{Enhancing Sampling of Biomolecular Simulations}
As described above, in an MD simulation, atomic motion is dictated by a potential energy function, and states are sampled with probabilities dictated by the Boltzmann distribution. Given an infinite amount of time, a simulation will sample all thermally-accessible states of a system's potential energy landscape. However, infinitely long simulations are not possible to achieve computationally.  Thus, because of the rugged energy landscapes common to biomolecular systems, many important states may not be sampled on the timescales of a typical simulation.
Furthermore, it is even more challenging to adequately sample the complex conformational landscape of disordered proteins because of the large number of conformational states that may need to be sampled in order to fully characterize their structural ensembles.\cite{Rauscher:2010p5682}

Generalized-ensemble algorithms are a class of algorithms that can be used with molecular simulations to enhance conformational sampling.  Simulated tempering (ST) and replica exchange (RE) are two well-known generalized-ensemble algorithms that can be used to enhance sampling by allowing simulations to diffuse in temperature-space, while obeying Boltzmann statistics (Equation~\ref{eq:boltzmann}).\cite{Marinari:1992p5428,Mitsutake:2001p5380} These algorithms rely on the underlying assumption that at higher temperatures the free energy landscape of a biomolecular system becomes less rugged, and, hence, the crossing of energetic barriers are facilitated.  A temperature range is typically chosen to include temperatures that are lower and higher than the room temperature (300 K), and is dependent on the size and composition of the system.\cite{Rauscher:2009wr,Rauscher:2010p5682} Although generalized-ensemble methods, when combined with MD simulations can greatly enhance conformational sampling of peptides, they are computationally expensive, and often require a parallel supercomputing platform to carry out a single study in a feasible time frame.  
...
Coordinates for the initial input into a simulation software need to be prepared appropriately before the MD simulation is performed.  First, the initial structure of the protein of interest need to be generated. If the target protein is an X-ray crystal or NMR structure, its structure can be downloaded from the Protein Databank (PDB). Next, the simulation environment is prepared to closely mimic the \emph{in vitro} experimental conditions. For example, because a crystal structure does not contain information about hydrogen atoms, one may need to manually assign protonation states for the basic and acidic residues in the protein. This can be done by predicting the pKa for each residue, and visually examining the structure for the presence of hydrogen bonds.

After the protein structure is prepared, the protein is usually solvated, with other molecules such as cosolutes, salt added to the simulation box. For more complex simulation systems, such as those of membrane proteins which require the presence of a lipid bilayer, additional steps need to be taken before the system is ready for simulating.
...
Energy minimization is usually performed on the system to remove any bad contacts that could have been introduced during system preparation. An example of a completed starting state of a simulation is shown in Figure~\ref{fig:simulation_box}.
...
Prior to performing production dynamics, the system is first equilibrated.  This equilibration step is usually a short simulation that is performed to allow the system to relax, and reach thermal equilibrium.  During this period, the system's temperature, pressure, and volume are closely monitored to ensure that they converge to their intended values. The length of the equilibration simulation is highly dependent on the system of interest.  A straightforward method for delineating the equilibration period for a simulation of a globular protein is to measure the time-evolution of the root-mean-square deviation (RMSD) of protein structure with respect to the crystal structure. Typically, only the data beyond the first time point at which the RMSD is observed to plateau is included in the final data analysis. 
