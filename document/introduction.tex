...
\section{Untitled: Rational drug design}
...
In order to find new drugs it is imperative to understand the mechanism of action of compounds at the molecular level. Structure-based rational drug design is a method that have yielded success in the discovery of lead compounds (Figure~\ref{fig:rational_drug_design}).  In paradigm, a target, often a protein in a relevant disease pathway, is first characterized using biochemical techniques.  Then, using a battery of structure determination techniques such as X-ray crystallography and NMR, the target protein structure is resolved at the atomic-level.   By having the molecular structure of,  enzymatic active sites or putative binding sites for ligands may be identified and binding may be characterized at the molecular-level.  At this stage of the pipeline,  high-throughput screening using a chemical compound library may be performed to obtain therapeutic candidates.  Once lead compounds are synthesized, in vitro experiments can be done to assess the structure-activity relationship of these compounds using kinetics and equilibria measurements.
...
Carbohydrate-binding proteins is a class of proteins where \KD's in the millimolar range (weak affinity binding) with their ligands (carbohydrates) are often found, but achieve binding specificity via multivalent binding to enable their function. 
...
A hydrogen bond is formed from electrostatic interactions formed between two dipoles (an acceptor and donor), where the acceptor group has an electronegative heavy atom (e.g. F, Cl, O), with exposed lone electron pairs, and the donor group is composed of an electropositive atom with an exposed proton.  Unlike dispersion forces, hydrogen bonds have directionality.

Hydrogen bonds are ubiquitous in water: a single water molecule is able to tetrahedrally coordinate four other water molecules to form a hydrogen bonding network, providing water with unique properties such as a high heat capacity. In proteins, intra- and intermolecular hydrogen bonds formed by the peptidic backbone of polypeptide chains  involves N-H and O=C as the donor and acceptor groups, respectively. Peptidic backbone hydrogen bonds define protein secondary structures (helices and \bsheets), and impart stability to their overall tertiary structures.\cite{refs}  In amyloid fibrils,  polypeptide backbones have their hydrogen bonded to each to form in-registered, parallel or antiparallel \bsheets\  (see Section \ref{sec:amyloid}).  Aside from the backbone, polar or charged amino acids () may also form hydrogen bonds.

\textbf{Salt bridges}

Energetically, hydrogen bonds can range from XXX to YYY kcal/mol, and are typically much stronger than van Der Waals forces.  The N-H ... C=O hydrogen bond has been estimated to be XXX kcal/mol in gas phase, and about 0.5 to 1.5 kcal/mol in solution.\cite{energetics of hydrogen bonds in peptides} A water-water hydrogen bond is estimated to be about -3.2 kcal/mol.\cite{where did I see this}
...
% Notes : Use the above to say why finding small-molecule inhibitors for amyloid is different.  Then mention why use molecular dynamics - say at a very high level what MD is.  Refer to the section for the detailed methods.
...
% Then these sections will provide a nice lead in to the methods section
