...
Although it is known that PgaB functions by binding to PNAG, the molecular basis of the mechanism of export of PNAG by PgaB is not currently not determined.  Moreover, the binding mode and the length of its substrate polymer are not known.
...
In support of this result, our previous study has shown that PgaB-CT is homologous to glycoside hydrolases,\cite{Little:2012dp} where the active site for these enzymes is also located in analogous grooves with a similar geometry. 
